...
import { captureLicense, checkCheckoutAvailability, checkoutUrl, type CheckoutAvailability } from "./license";
...
const footer = `<footer class="site-footer"><p><strong>Presence Bridge</strong><br><span>See who is free, then open your existing tool.</span></p><nav aria-label="Footer navigation"><a href="/privacy" data-route>Privacy</a><a href="/terms" data-route>Terms</a><a href="https://sociobot.in" rel="external">Built by Param Factory</a></nav><p>v0.1.7 · Original generated artwork</p></footer>`;
...
const home = (): string => `<div class="site-page home-page">${header}<main id="main">
  ...
  <section class="boundary" aria-labelledby="boundary-title"><div class="boundary-art" aria-hidden="true"><span></span><span></span><span></span></div><div><p class="eyebrow">A quiet boundary</p><h2 id="boundary-title">Share status, not surveillance</h2><p>Download a presence update when you want teammates to see it. It carries only your chosen status fields. Presence Bridge does not carry messages or infer activity.</p></div></section>
  <section class="price-strip" aria-labelledby="price-title"><div><p class="eyebrow">Bridge Plus · $24 once when available</p><h2 id="price-title">More room, same quiet roster</h2><p>Add up to ten people and more contact routes. The free local roster holds five.</p></div><div id="checkout-actions" class="checkout-actions" aria-live="polite">${checkoutAction("unavailable", "site")}</div></section>
</main>${footer}</div>`;
...
function checkoutAction(state: CheckoutAvailability | "checking", surface: "site" | "app"): string {
  if (state === "available") return `<a class="primary-link" href="${checkoutUrl}">Buy Bridge Plus</a><p>Sociobot handles payment and licenses.</p>`;
  ...
  const context = surface === "site" ? "The free five-person roster is ready now." : "You can still restore an existing license below.";
  return `<p>Bridge Plus purchases are not available right now. ${context}</p><button class="secondary" type="button" data-action="check-checkout">Check whether Bridge Plus is available</button>`;
}
...
function navigate(path: string, push = true): void {
  ...
  root.querySelectorAll<HTMLButtonElement>('[data-action="check-checkout"]').forEach(button => button.addEventListener("click", () => void checkCheckout(root)));
  route.after?.();
...
async function checkCheckout(root: HTMLElement): Promise<void> {
  const actions = root.querySelector<HTMLElement>("#checkout-actions");
  if (!actions) return;
  actions.innerHTML = checkoutAction("checking", "site");
  const availability = await checkCheckoutAvailability();
  if (!actions.isConnected) return;
  actions.innerHTML = checkoutAction(availability, "site");
  actions.querySelector<HTMLButtonElement>('[data-action="check-checkout"]')?.addEventListener("click", () => void checkCheckout(root));
}
